...
import java.util.*;
	...
	public class BinaryTree <T> {
	    ...
	    public void imprimirArbolPreOrden() {    //PreOrden
	    	System.out.println(this.getData() + " ");
	        if(this.hasLeftChild()) this.getLeftChild().imprimirArbolPreOrden();
	        if(this.hasRightChild()) this.getRightChild().imprimirArbolPreOrden();
	    }
		...
		public void entreNivelesInfo(int n, int m) {
		    if (this.isEmpty() || n < 0 || m < n) return; 
		    Queue<BinaryTree<T>> cola = new LinkedList();
		    cola.add(this);
		    int nivelActual = 0;
		...
		public void agregarMio(BinaryTree<T> arbol , BinaryTree<T> hijo) { //sirve si tenemos en cuenta el orden
			if(arbol.isEmpty()) {
				arbol.setData(hijo.getData());				
			}
			else
			{
				if((Integer)hijo.getData() < (Integer)arbol.getData()) {
					if(arbol.hasLeftChild()) {
						agregarMio(arbol.getLeftChild(),hijo);
					}
					else
					{
				...
				else
				{
					if(arbol.hasRightChild()) {
						agregarMio(arbol.getRightChild(),hijo);
					}
					else
					{
		...
		//crear uno ahora que no le importe el orden, preguntar
	}
